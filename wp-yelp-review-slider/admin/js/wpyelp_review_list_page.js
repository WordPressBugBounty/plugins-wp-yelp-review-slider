(function( $ ) {
	...
	$(function(){
		...
		$( "#wpyelp_removeallbtn" ).click(function() {
		  ...
		  openpopup("Are you sure?", '<p>This will delete all reviews in your Wordpress database including the ones you manually entered. It Does NOT affect your reviews on Facebook.</p>', '<a class="button dashicons-before dashicons-no" href="?page=wp_yelp-reviews&opt=delall&_wpnonce='+sec+'">Remove</a>');
		});	

		//upgrade to pro
		$( ".wpyelp_upgrade_needed" ).click(function() {
		  openpopup("Upgrade Needed", '<p>Please upgrade to the Pro Version of this Plugin to access this feature.</p>', '<a class="button dashicons-before  dashicons-cart" href="?page=wp_yelp-get_pro">Upgrade Here</a>');
		});		
		...
		$( "#wpyelp_addnewreviewbtn" ).click(function() {
		  jQuery("#wpyelp_new_review").show("slow");
		});	
		$( "#wpyelp_addnewreview_cancel" ).click(function() {
		  jQuery("#wpyelp_new_review").hide("slow");
		  //reload page without taction and tid
		  setTimeout(function(){ 
			window.location.href = "?page=wp_yelp-reviews"; 
		  }, 500);
		...
		window.send_to_editor = function(html) {
			...
			$('#wpyelp_nr_avatar_url').val(image_url);
		...
		$("#newreviewform").submit(function(){ 
			  ...
			  if ($('input[name=wpyelp_nr_rating]:checked').length) {
			...
			if(jQuery( "#wpyelp_nr_name").val()==""){
				alert("Please enter a name.");
				$( "#wpyelp_nr_name" ).focus();
				return false;
			} else {
				return true;
			}
		...
		$("#review_list").on("click", ".reveditbtn", function (event) {
			...
			$("#wpyelp_new_review").show("slow");
			//find values from rowobject and fill in edit form wpyelp_nr_rating, wpyelp_nr_text, wpyelp_nr_name, wpyelp_nr_avatar_url, wpyelp_nr_date
			$("#editrid").val(rid);
			$("#wpyelp_nr_name").val(name);
			$("#wpyelp_nr_avatar_url").val(wprev_row_userpic);
			//for radio
			$("#wpyelp_nr_rating").val(wprev_row_rating);
			$("#wpyelp_nr_date").val(wprev_row_created_time);
			
			$("#wpyelp_nr_text").val(wprev_row_review_text);
		...
		function sendtoajax(rid,whattodo,rowobject){
			var senddata = {
					action: 'wpyelp_hide_review',	//required
		...
		//for pagination bar-----------------------------------
		$("#wpyelp_review_list_pagination_bar").on("click", "span", function (event) {
		...
		function sendtoajaxreview(pageclicked,sortbyval,sortd,selrevs){
			...
			$( "#review_list" ).html("");
			$( "#wpyelp_review_list_pagination_bar" ).html("");
			var senddata = {
				...
				var reviewtext = '';
					...
					$.each(object, function(index) {
						...
						if(object[index].reviewer_name){
							...
							if(object[index].type=="Facebook"){
								...
								editdellink ='';
							} else if(object[index].type=="Yelp"){
					...
					if(numpages>1){
						...
						$( "#wpyelp_review_list_pagination_bar" ).html(pagebarhtml);

					if(reviewtotalcount==0){
						$("#wpyelp_review_list_pagination_bar").hide();
					} else {

						$("#wpyelp_review_list_pagination_bar").show();
					}
