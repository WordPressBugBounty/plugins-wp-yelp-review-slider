	...
	$(function(){
		...
		function insertoverallchart(tempdataset){
			...
			$('#overallChart').click(function (e)
				{
					var activePoints = myChartOverall.getElementsAtEvent(e);
					...
					if (activePoints.length > 0)
					{
						...
						var reviewtexthtml = '';
						var clickedrev = tempdataset.reviews[clickedElementIndex];
						if(clickedrev.review_title){
							reviewtexthtml="<b>"+clickedrev.review_title+"</b><br>"+(clickedrev.review_text || '');
						} else {
							reviewtexthtml=clickedrev.review_text || '';
						}
						if(clickedrev.owner_response){
							try {
								var ownerres=JSON.parse(clickedrev.owner_response);
		...
		function buildwordclouds(type){
			...
			$.each( tempglobal, function( key, value ) {
				...
				var tempwordobj = {text: key, weight: value, handlers: {click: function() {showclickedwordreviews(this, type);}}};
		...
		function showclickedwordreviews(clickedword, cloudtype){
			console.log(clickedword);
			var clickedwordtext = (($(clickedword).text() || '') + '').toLowerCase().trim();
			var clickedspanid = $(clickedword).attr('id') || '';
			var posornegrating = 'neg';
			if(cloudtype === 'pos' || clickedspanid.indexOf('positive') !== -1){
				posornegrating = 'pos';
			}
			...
			$.each( tempdataset.reviews, function( key, value ) {
				...
				if(clickedwordtext && value.review_text && lowercasereviewtext.indexOf(clickedwordtext) !== -1){
