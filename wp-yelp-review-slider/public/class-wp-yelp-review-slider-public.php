 ...
 * @subpackage WP_Yelp_Review/public
 */
...
class WP_Yelp_Review_Public {
		...
		foreach ( $templatearray as $template ){
			if(isset($template->style)){
				if($template->style=="1" || $template->style=="2" || $template->style=="3" || $template->style=="4" || $template->style=="5" || $template->style=="6" || $template->style=="7" || $template->style=="8" || $template->style=="9" || $template->style=="10"){
					wp_register_style( 'wp-yelp-review-slider-public_template'.$template->style, plugin_dir_url( __FILE__ ) . 'css/wprev-public_template'.$template->style.'.css', array(), $this->version, 'all' );
					wp_enqueue_style( 'wp-yelp-review-slider-public_template'.$template->style );
				}
			}
		}

		wp_register_style( 'wpyelp_w3', plugin_dir_url( __FILE__ ) . 'css/wpyelp_w3.css', array(), $this->version, 'all' );
	 ...
	 * @since    1.0.0
	 */
	public function shortcode_wpyelp_usetemplate() {
	
				add_shortcode( 'wpyelp_usetemplate', array($this,'wpyelp_usetemplate_func') );
	}	 
	public function wpyelp_usetemplate_func( $atts, $content = null ) {
		//get attributes
		    $a = shortcode_atts( array(
				...
				ob_start();
				include plugin_dir_path( __FILE__ ) . '/partials/wp-yelp-review-slider-public-display.php';
				return ob_get_clean();
	}
}
